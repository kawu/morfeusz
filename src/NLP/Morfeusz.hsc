...
-- interpretations is empty, the token is unknown to Morfeusz.
data Token = Token
    { orth      :: T.Text
    , interps   :: [Interp] }
    deriving (Show)
...
analyseRaw :: KeepSpaces -> T.Text -> [Edge RawInterp]
...
-- a DAG labeled with tokens.
properDAG :: [Edge RawInterp] -> [Edge Token]
properDAG dag =
...
analyse :: T.Text -> DAG () Token
analyse = fromEdges . properDAG . analyseRaw False
